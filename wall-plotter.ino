...
void drawLine(long distanceL, long distanceR){
    ...
    for (long i = 1; i <= ticks; i++) {
        if (i % distL == 0) {
            motorL.step(STEPS_PER_TICK * directionL);
        }
        if (i % distR == 0) {
            motorR.step(STEPS_PER_TICK * directionR);
        }
    }
}

void getDistance(float x, float y, long *distanceLeft, long *distanceRight) {
    float nextX = x * zoom + lastX;
    float nextY = y * zoom + lastY;
    float xPos1 = nextX + centerX;
    float xPos2 = nextX + centerX - canvasWidth;
    float yPos  = nextY + centerY;
    long newXPos1  = sqrt(pow(xPos1, 2) + pow(yPos, 2));
    long newXPos2 = sqrt(pow(xPos2, 2) + pow(yPos, 2));
    *distanceRight  = (newXPos1  - currentLeft);
    *distanceLeft = (newXPos2 - currentRight);
    currentLeft = newXPos2;
    currentRight = newXPos1;
    lastX = nextX;
    lastY = nextY;
}
